...
public class Cavern {
    ...
    private static int generateGoldValue(Random rand) {
        ...
        if (val == MAX_GOLD_VALUE) val= TASTY_VALUE;
        return val;
    }
    ...
    private Cavern(int rws, int cls, Random rand,
        ...
        for (int i= 0; i < rows; i++ ) for (int j= 0; j < cols; j++ )
            if (tiles[i][j] == null) tiles[i][j]= new Node(new Tile(i, j, 0, Tile.Type.WALL), cols);
    ...
    private void createEdges(Node[][] t, Supplier<Integer> edgeWeightGenerator) {
        for (int i= 0; i < t.length - 1; i++ ) for (int j= 0; j < t[i].length - 1; j++ ) {
            Node node= t[i][j];
            if (node.getTile().getType() == Tile.Type.WALL) continue;
    ...
    private Set<Node> generateGraph(Random rand,
        ...
        while (!frontier.isEmpty()) {
            ...
            for (Direction dir : Direction.values()) {
                ...
                if (isValid(newPt)) if (openPoints.contains(newPt)) existingExits++ ;
                else if (pointsSeen.add(newPt)) newExits.add(newPt);
            }
    ...
    public static Cavern deserialize(List<String> nodeStrList) {
        ...
        for (String nodeStr : nodeStrList) {
            if (nodeStr.equals(extraInfo)) continue;
